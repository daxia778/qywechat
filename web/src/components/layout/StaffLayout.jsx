              ...
              {showSoundSettings && (
                ...
                >
                  ...
                  <div className="px-4 py-3 space-y-1.5">
                    ...
                    {SOUND_OPTIONS.map((opt) => (
                      <div
                        ...
                        onClick={() => { setSoundType(opt.value); previewSound(opt.value, 'normal', volume); }}
                      >
                        <div className="flex items-center gap-2.5">
                          <span className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors ${
                            ...
                            {soundType === opt.value && <span className="w-2 h-2 rounded-full bg-[#434fcf]" />}
                          </span>
                          <span className={`text-[13px] ${soundType === opt.value ? 'font-semibold text-[#434fcf]' : 'text-slate-600'}`}>{opt.label}</span>
                        </div>
                        <button
                  ...
                  <div className="px-4 py-3 border-t border-slate-100 flex gap-2">
                    <button onClick={() => previewSound(soundType, 'normal', volume)} className="flex-1 px-3 py-2 text-xs font-semibold rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-600 transition-colors">普通提示音</button>
                    <button onClick={() => previewSound(soundType, 'urgent', volume)} className="flex-1 px-3 py-2 text-xs font-semibold rounded-lg bg-red-50 hover:bg-red-100 text-red-600 transition-colors">紧急提示音</button>
                  </div>
                </div>
              )}
            </div>
