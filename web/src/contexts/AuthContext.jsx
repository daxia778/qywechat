...
export function AuthProvider({ children }) {
  ...
  // 兼容 staff 页面通过 user.role / user.name 访问
  const user = useMemo(() => ({
    role, name: userName, username: userId, wecom_userid: userId,
  }), [role, userName, userId]);

  const value = useMemo(() => ({
    token, userName, userId, role, isAuthenticated, ready, login, logout, checkToken, user
  }), [token, userName, userId, role, isAuthenticated, ready, login, logout, checkToken, user]);
