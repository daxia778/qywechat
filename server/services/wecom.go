...
// ─── 客户群管理（externalcontact/groupchat API） ──────────────────────────

// UpdateGroupChat 修改客户群设置（群名/群公告/禁止互加联系人）
func (w *WeComClient) UpdateGroupChat(chatID string, opts map[string]any) error {
	if !w.contactConfigured {
		return fmt.Errorf("客户联系功能未配置")
	}
	...
	url := fmt.Sprintf("%s/externalcontact/groupchat/update?access_token=%s", w.baseURL, token)
	...
	respBytes, err := w.postJSONRaw(url, reqBody)
	if err != nil {
		return fmt.Errorf("修改客户群失败: %w", err)
	}
	var result struct {
		ErrCode int    `json:"errcode"`
		ErrMsg  string `json:"errmsg"`
	...
	if err := json.Unmarshal(respBytes, &result); err != nil {
		return fmt.Errorf("解析修改客户群响应失败: %w", err)
	}
	if result.ErrCode != 0 {
		return fmt.Errorf("修改客户群失败: errcode=%d errmsg=%s", result.ErrCode, result.ErrMsg)
	}
	log.Printf("客户群设置已更新 | chatid=%s | opts=%v", chatID, opts)
	return nil
}

// SetGroupName 设置客户群群名
func (w *WeComClient) SetGroupName(chatID, name string) error {
	return w.UpdateGroupChat(chatID, map[string]any{"name": name})
}

// SetGroupNotice 设置客户群公告
func (w *WeComClient) SetGroupNotice(chatID, notice string) error {
	return w.UpdateGroupChat(chatID, map[string]any{"notice": notice})
}

// SetGroupAddMemberPermit 设置禁止互加联系人（1=禁止，0=允许）
func (w *WeComClient) SetGroupAddMemberPermit(chatID string, forbid bool) error {
	permit := 0
	if forbid {
		permit = 1
	}
	return w.UpdateGroupChat(chatID, map[string]any{"add_member_permit": permit})
}

// TransferGroupOwner 转让客户群群主
func (w *WeComClient) TransferGroupOwner(chatIDList []string, newOwner string) ([]map[string]any, error) {
	if !w.contactConfigured {
		return nil, fmt.Errorf("客户联系功能未配置")
	}
	token, err := w.GetContactAccessToken()
	...
	url := fmt.Sprintf("%s/externalcontact/groupchat/transfer?access_token=%s", w.baseURL, token)
	reqBody := map[string]any{
	...
	respBytes, err := w.postJSONRaw(url, reqBody)
	if err != nil {
		return nil, fmt.Errorf("转让群主失败: %w", err)
	}
	var result struct {
		ErrCode    int              `json:"errcode"`
		ErrMsg     string           `json:"errmsg"`
		FailedList []map[string]any `json:"failed_chat_list"`
	}
	if err := json.Unmarshal(respBytes, &result); err != nil {
		return nil, fmt.Errorf("解析转让群主响应失败: %w", err)
	}
	if result.ErrCode != 0 {
		return nil, fmt.Errorf("转让群主失败: errcode=%d errmsg=%s", result.ErrCode, result.ErrMsg)
	}
	log.Printf("群主转让完成 | chatids=%v | new_owner=%s | failed=%d", chatIDList, newOwner, len(result.FailedList))
	return result.FailedList, nil
}

// ConfigureNewGroup 建群后一键配置（设群名 + 禁止互加联系人 + 转让群主）
func (w *WeComClient) ConfigureNewGroup(chatID, groupName, newOwner string) error {
	// 设置群名
	if groupName != "" {
		if err := w.SetGroupName(chatID, groupName); err != nil {
			log.Printf("设置群名失败: %v", err)
		} else {
			log.Printf("群名已设置: %s", groupName)
		}
	}
	// 禁止互加联系人
	if err := w.SetGroupAddMemberPermit(chatID, true); err != nil {
		log.Printf("设置禁止互加联系人失败: %v", err)
	} else {
		log.Printf("已禁止群内互加联系人")
	}
	// 转让群主
	if newOwner != "" {
		failedList, err := w.TransferGroupOwner([]string{chatID}, newOwner)
		if err != nil {
			log.Printf("转让群主失败: %v", err)
		} else if len(failedList) > 0 {
			log.Printf("转让群主部分失败: %v", failedList)
		} else {
			log.Printf("群主已转让给: %s", newOwner)
		}
	}
	return nil
}
