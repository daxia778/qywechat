...
type CustomerTransfer struct {
	...
	Status         string     `gorm:"column:status;size:16;default:pending" json:"status"`             // pending/waiting/success/failed/scheduled
	...
	ScheduledTime  *time.Time `gorm:"column:scheduled_time" json:"scheduled_time,omitempty"`          // 定时转接时间（空=立即）
