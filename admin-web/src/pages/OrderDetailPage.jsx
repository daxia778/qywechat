        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-[0_1px_3px_rgba(0,0,0,0.04),0_1px_2px_rgba(0,0,0,0.02)]">
          <div className="px-6 py-4 flex items-center gap-3 flex-wrap">
            ...
            {order.status === 'PENDING' && canOperate('operator') && (
              <button onClick={() => doUpdateStatus('GROUP_CREATED')} className="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl text-white bg-[#465FFF] hover:bg-[#3641F5] transition-all duration-150 cursor-pointer border-none shadow-sm text-[12px]">确认建群</button>
            )}
            {order.status === 'GROUP_CREATED' && !order.designer_id && canOperate('operator') && (
              <button onClick={() => doUpdateStatus('DESIGNING')} className="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl text-white bg-[#465FFF] hover:bg-[#3641F5] transition-all duration-150 cursor-pointer border-none shadow-sm text-[12px]">分配设计</button>
            )}
            {order.status === 'DESIGNING' && canOperate('designer') && (
              <button onClick={() => doUpdateStatus('DELIVERED')} className="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl transition-all duration-150 cursor-pointer border border-[#22AD5C] text-[#22AD5C] hover:bg-[#DAF8E6] text-[12px] bg-white">标记交付</button>
            )}
            {order.status === 'DELIVERED' && canOperate('operator') && (
              <button onClick={() => showModal({
                ...
                detail: { '订单号': order.order_sn, '金额': `¥${(order.price / 100).toFixed(2)}` },
              }, () => doUpdateStatus('COMPLETED'))} className="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl text-white bg-[#465FFF] hover:bg-[#3641F5] transition-all duration-150 cursor-pointer border-none shadow-sm text-[12px]">标记完成</button>
            )}
            {canOperate('operator') && (
              <>
                <button onClick={() => showModal({
                ...
                })} className="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl transition-all duration-150 cursor-pointer border border-amber-400 text-amber-600 hover:bg-amber-50 text-[12px] bg-white">退款/售后</button>
                {role === 'admin' && ['PENDING','GROUP_CREATED','DESIGNING'].includes(order.status) && (
                  ...
                  }, () => doUpdateStatus('CLOSED'))} className="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl text-white bg-[#F04438] hover:bg-red-600 transition-all duration-150 cursor-pointer border-none shadow-sm text-[12px]">关闭订单</button>
                )}
              </>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 flex flex-col gap-6">
            ...
            <div className="px-5 lg:px-7 py-5 border-b border-slate-200 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-[#EFF4FF]">
                  <svg className="w-4 h-4 text-[#465FFF]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-[0_1px_3px_rgba(0,0,0,0.04),0_1px_2px_rgba(0,0,0,0.02)]">
            <div className="px-5 lg:px-7 py-5 border-b border-slate-200 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-[#DAF8E6]">
                  <svg className="w-4 h-4 text-[#22AD5C]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                </div>
            ...
            <div className="p-6">
              {timeline.length === 0 ? (
                <div className="text-center text-sm text-slate-400 py-10">
                  <div className="animate-spin rounded-full h-6 w-6 border-2 border-slate-200 border-t-[#465FFF] mx-auto mb-3" />
                  加载中...
                </div>
              ) : (
                <div className="relative pl-6">
                  <div className="absolute left-[9px] top-2 bottom-2 w-0.5 bg-gradient-to-b from-[#465FFF]/20 via-slate-200 to-slate-100" />
                  {timeline.map((event, i) => (
                    <div key={i} className="relative mb-6 last:mb-0">
                      <div className={`absolute -left-6 top-0.5 w-[18px] h-[18px] rounded-full border-2 flex items-center justify-center transition-all ${
                        i === timeline.length - 1
                          ? 'border-[#465FFF] bg-[#465FFF] shadow-sm shadow-[#465FFF]/30'
